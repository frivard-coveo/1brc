...
// This program is a re-write in C# of the original Java program : https://github.com/gunnarmorling/1brc/blob/main/src/main/java/dev/morling/onebrc/CreateMeasurements.java
class Program
{
    private static string MEASUREMENT_FILE = "measurements.txt";
    ...
    static int Main(string[] args)
    {
        ...
        int size = 0;
        var parseSuccess = int.TryParse(args[0], out size);
        if(!parseSuccess)
        {
        ...
        using(var file = File.CreateText(MEASUREMENT_FILE))
        {
            for (int i = 0; i < size; i++)
            {
                if (i > 0 && i % 50_000_000 == 0)
                {
                    Console.WriteLine($"Wrote {i} measurements in {sw.ElapsedMilliseconds} ms");
                }
        ...
        Console.WriteLine($"Created file with {size} measurements in {sw.ElapsedMilliseconds} ms");
        return 0;
    }
}
...
public static class RandomExtensions
{
    ...
    public static double NextGaussian(this Random random, double mean, double stdDev)
    {
        if (haveNextNextGaussian)
        {
            ...
            return mean + stdDev * nextNextGaussian;
        }
        else
        {
            double v1, v2, s;
